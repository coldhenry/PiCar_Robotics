# -*- coding: utf-8 -*-
"""
Created on Sun Nov 17 16:27:22 2019
...
def VRM_planning(sx, sy, gx, gy, ox, oy, rr):
    ...
    # voronoi points generated by the current obstacles
...
def dijkstra_planning(sx, sy, gx, gy, ox, oy, rr, road_map, sample_x, sample_y):
    ...
    while pind != -1:
        ...
        pind = n.pind
    s_pts = 10
    print(rx[::s_pts],ry[::s_pts])
    return rx[::s_pts], ry[::s_pts]

def Obstacle_Map(sx,sy,mx,my,gx,gy,show_animation):
    ox = [] # original coordinates
    oy = []
    cx = [] # C-space coordinates
    cy = []
    # 4 edges of map
    for i in range(96):
        ox.append(i)
    ...
    if show_animation:  # pragma: no cover
        lines= plt.plot(ox, oy, ".y")
        plt.setp(lines,linewidth=30)
        plt.plot(cx, cy, ".k")
        plt.plot(sx, sy, "^r")
        plt.plot(gx, gy, "^c")
        plt.plot(mx, my, "^g")
